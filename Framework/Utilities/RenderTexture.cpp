...
MipMap
- DirectX에서 아래의 목적을 위해서 텍스처의 여러 해상도 버전을 포함하는 이미지 컬렉션,
  각각의 해상도 버전은 배열로 저장
- 텍스처 매핑이 효율적으로 이루어짐
    - 3D 공간에서 객체가 화면에 가까울수록 크게 보이고, 멀리 있을수록 작게 보인다
    - 가까운 객체에는 높은 해상도의 텍스처를 매핑하고, 멀리 있는 객체에는 낮은 해상도의 텍스처를 매핑
        -> 자원 사용량이 줄어들고 렌더링 속도가 빨라진다
*/